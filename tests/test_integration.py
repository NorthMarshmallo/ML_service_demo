...
def test_prediction_is_logged(client, good_row):
    ...
    with psycopg.connect(DATABASE_URL) as conn:
        row = conn.execute(
             "SELECT model_version, score, features->>'sequence', status_code "
    ...
    assert row[2] == good_row["sequence"]
    assert row[3] == 200


def test_validation_error_is_logged(client):
    bad_row = {"sequence": "garbage123"}
    request_id = client.post("/v1/predict", json=bad_row).headers["X-Request-ID"]
